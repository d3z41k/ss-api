...
async function extra() {
  return new Promise(async (resolve, reject) => {
    ...
    async function start(auth) {
      ...
      range = list + '!A1:U';
      let extraClients = await crud.readData(config.sid_2017_2.extra, range);
      ...
      try {
        ...
        await Promise.all([
          crud.updateData(values[0], config.sid_2017_2.extra, prePayRange),
          crud.updateData(values[1], config.sid_2017_2.extra, addPayRange)
        ])
