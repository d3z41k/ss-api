...
async function amoReg() {
  return new Promise(async(resolve, reject) => {
    ...
    async function start(auth) {
      ...
      range = list.amo + '!A1:A';
        let xLable = await crud.readData(config.sid_2017_2.amo, range);
        let xArray = [];
      ...
      let registryData = await crud.readData(config.sid_2017_2.amo, range);
      ...
      let clientData = await crud.readData(config.sid_2017_2.amo, range);
      ...
      for (let x = 0; x < xArray.length; x++) {
        ...
        for (let i = 0; i < monthAct.length; i++) {
          if (registryData[xArray[x] - START][0]  == monthAct[i][0]) {
            ...
            await crud.updateData([[monthAct[i][2]]], config.sid_2017_2.amo, range)
      ...
      await crud.updateData(endMonths, config.sid_2017_2.amo, range)
      //  .then(async result => {console.log(result);})
        .catch(console.err);
      ...
      await crud.updateData(zipValues, config.sid_2017_2.amo, range)
      //  .then(async result => {console.log(result);})
        .catch(console.err);
       ...
       ************************************************************************/
       ...
       try {
         ...
         zipValues.forEach((arrValues, i)=> {
           arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.amo, arrRange[i]));
         });
      ...
      ratio.forEach((arrValues, i)=> {
        arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.amo, arrRange.ratio[i]));
      });
      ...
      factHours.forEach((arrValues, i)=> {
        arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.amo, arrRange.factHours[i]));
      });
      ...
      ************************************************************************/
      ...
      await Promise.all([
        crud.updateData(marginAll, config.sid_2017_2.amo, range1),
        crud.updateData(marginsAll, config.sid_2017_2.amo, range2),
      ])
      //  .then(async result => {console.log(result);})
        .catch(console.err);
