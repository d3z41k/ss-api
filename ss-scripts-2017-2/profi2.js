...
async function profi2(months) {
  return new Promise(async (resolve, reject) => {
    ...
    let colMonths = config.profi_MonCols_2017_2;
    let nowMonths  = {};
    let mode = 0;
    ...
    async function start(auth) {
      ...
      try {
        ...
        dataProfi = await crud.readData(config.sid_2017_2.profi2, range);
        ...
        dataKZ = await crud.readData(config.sid_2017_2.profi2, rangeKZ);
        ...
        for (let d = 0; d < directions.length; d++){
          ...
          if (directions[d] == 'КЗ') {
            ...
            dataKZ = await crud.readData(config.sid_2017_2.profi2, range);
            ...
            zipValues.forEach((arrValues, i) => {
              arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.profi2, arrRange[i]));
            });
            ...
            zipValues3.forEach((arrValues, i)=> {
              arrFuncions3.push(crud.updateData(arrValues, config.sid_2017_2.profi2, arrRange3[i]));
            });
