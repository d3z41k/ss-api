...
async function extraReg() {
  return new Promise(async (resolve, reject) => {
    ...
    async function start(auth) {
      ...
      const COL_MONTH = config.reg_colMonths_2;
      const COL_ADD_COSTS = config.addCosts_2;
      ...
      let xLable = await crud.readData(config.sid_2017_2.extra, range);
      let xArray = [];
      ...
      let registryData = await crud.readData(config.sid_2017_2.extra, range);
      ...
      let clientData= await crud.readData(config.sid_2017_2.extra, range);
      ...
      for (let x = 0; x < xArray.length; x++) {
        ...
        for (let i = 0; i < monthAct.length; i++) {
          if (registryData[xArray[x] - START][0]  == monthAct[i][0]) {
            ...
            await crud.updateData([[monthAct[i][2]]], config.sid_2017_2.extra, range)
      ...
      await crud.updateData(endMonths, config.sid_2017_2.extra, range)
        .then(async result => {console.log(result);})
        .catch(console.err);
      ...
      await crud.updateData(zipValues, config.sid_2017_2.extra, range)
      //  .then(async result => {console.log(result);})
        .catch(console.err);
      ...
      try {
        ...
        zipValues.forEach((arrValues, i)=> {
          arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.extra, arrRange[i]));
        });
      ...
      try {
        ...
        receiptParams[2] = [7, 8, 9, 10, 11, 12];
      ...
      ratio.forEach((arrValues, i) => {
        arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.extra, arrRange.ratio[i]));
      });
      ...
      factHours.forEach((arrValues, i) => {
        arrFuncions.push(crud.updateData(arrValues, config.sid_2017_2.extra, arrRange.factHours[i]));
      });
      ...
      //------------------------------------------------------------------------
       ...
       await Promise.all([
         crud.updateData(marginAll, config.sid_2017_2.extra, range1),
         crud.updateData(marginsAll, config.sid_2017_2.extra, range2),
       ])
         .then(async result => {console.log(result);})
         .catch(console.err);
