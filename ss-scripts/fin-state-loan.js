      ...
      for (let month in COL_MONTH) {
        ...
        for (let d = 0; d < indexBalance.length; d++) {
          ...
          for (let l = 4; l < 9; l++) {
            ...
            let value;
            loan[l][indexBalance[d]] ? value = loan[l][indexBalance[d]].trim() : value = 0;
        ...
        range1 = list + '!' + COL_MONTH[month][0] + '5:' + COL_MONTH[month][COL_MONTH[month].length - 1] + '9';
        range2 = list + '!' + COL_MONTH[month][0] + '10:' + COL_MONTH[month][COL_MONTH[month].length - 1] + '14';

        await Promise.all([
          crud.updateData(sumLoan, config.ssId.fin_state, range1),
          crud.updateData(sumRefund, config.ssId.fin_state, range2)
        ])
