...
async function domain() {
  return new Promise(async (resolve, reject) => {
    ...
    const domainQuery = require('../models-2017-1/db_domain-query');
    ...
    async function start(auth) {
      ...
      //normLength(srcRows);
       ...
       await dbRefresh(pool, 'dds_olga', srcRows)
        ...
        range = list + '!A1:AA';
        let domainClients = await crud.readData(config.tid2017.domain, range);
        ...
        for (let m = MONTHS[0]; m <= MONTHS[MONTHS.length - 1]; m++) {
          ...
          //console.log(paramsDomainCients);
          ...
          await Promise.all([
            crud.updateData(values[0][0], config.tid2017.domain, sellPayRangeD),
            crud.updateData(values[0][1], config.tid2017.domain, prePayRangeD),
            crud.updateData(values[0][2], config.tid2017.domain, addPayRangeD),
            crud.updateData(values[1][0], config.tid2017.domain, sellPayRangeH),
            crud.updateData(values[1][1], config.tid2017.domain, prePayRangeH),
            crud.updateData(values[1][2], config.tid2017.domain, addPayRangeH)
          ])
