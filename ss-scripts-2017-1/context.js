    ...
    async function start(auth) {
      ...
      //normLength(dataDDS);
       ...
       await dbRefresh(pool, 'dds_olga', dataDDS)
        ...
        range = list + '!A1:AB';
        let contextClients = await crud.readData(config.sid_2017.context, range);
        ...
        for (let m = MONTHS[0]; m <= MONTHS[MONTHS.length - 1]; m++) {
          ...
          await Promise.all([
            crud.updateData(values[0], config.sid_2017.context, sellPayRange),
            crud.updateData(values[1], config.sid_2017.context, prePayRange),
            crud.updateData(values[2], config.sid_2017.context, finalPayRange),
            crud.updateData(values[3], config.sid_2017.context, contextPayRange),
          ])
